...
import { newLegalRequest } from "@/core/endpoints";
...
export const useDashboardStore = defineStore('DashboardStore', {
    ...
    actions: {
        async fill() {
            ...
            try {
                const result = await axios.get(`${newLegalRequest}${urlParams ? `?${ urlParams }`:''}`);
                const { data, meta } = getItemAndMetaData(result.data);
                this.can = result.data.can;
                this.meta = meta;
                this.legalRequests = data;
                this.cardData = result.data.can.cardData;
            } catch (error) {
        ...
        async handleSubmit() {
            try {
                ...
                const { data } = await axios.put(`${newLegalRequest}/${id}`, { completed_date, additional_lawyer_note, billed_amount });
        ...
        archived(legalReqId) {
            confirm("Do you want to archived this request?", {confirmButtonText: 'Yes, archived it!'}).then(isConfirm => {
                if (isConfirm) {
                    ...
                    axios.post(`${newLegalRequest}/${legalReqId}/archived`).then(({ data }) => {
                        flash(data.message);
                        this.loading = false;
